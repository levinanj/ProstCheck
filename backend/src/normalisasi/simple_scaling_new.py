...
data_cleaned = pd.read_excel(input_file_path)
...
data_normalized = pd.concat([X_scaled, data_cleaned['diagnosis_result']], axis=1)

# 3. Ekspor hasil normalisasi ke file Excel
data_normalized.to_excel(output_file_path_normalized, index=False)
